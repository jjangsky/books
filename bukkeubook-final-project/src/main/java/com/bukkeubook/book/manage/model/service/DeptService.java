...
import com.bukkeubook.book.manage.model.entity.DeptAndEmp;
...
public class DeptService {
	...
	public List<DeptAndEmpDTO> selectDeptList() {
		...
		List<DeptAndEmp> deptList = new ArrayList<DeptAndEmp>();
		
		deptList = deptRepository.findAll(Sort.by("deptCode"));
		
		return deptList.stream().map(DeptAndEmp -> modelMapper.map(DeptAndEmp, DeptAndEmpDTO.class)).collect(Collectors.toList());
	}

	@Transactional
	public DeptAndEmpDTO searchDeptDetail(int deptCode) {
		DeptAndEmp dept = deptRepository.findById(deptCode).get();
